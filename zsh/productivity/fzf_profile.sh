...
fdata-profile() {
  ...
  if [[ "$multi_profile_mode" == "true" ]]; then
    ...
    # Display multi-profile execution plan
    echo "✅ Selected profiles (${#selected_profiles[@]}): ${(j:, :)selected_profiles}"
    echo "📁 Files: ${(j:, :)files}"
    echo "🔄 Would run ${#selected_profiles[@]} profile(s) on ${#files[@]} file(s):"
    
    local counter=1
    for profile in "${selected_profiles[@]}"; do
      if [[ "$profile" == "🔄 "* ]]; then
        local suite_name="${profile#🔄 }"
        echo "   $counter. batch suite '$suite_name' → ${(j:, :)files}"
      else
        echo "   $counter. $profile → ${(j:, :)files}"
      fi
      ((counter++))
    done
    ...
    if [[ "$selected" == "🔄 "* ]]; then
      ...
      echo "🔄 Would run profile '$selected' on ${#files[@]} file(s)"
    fi
  fi
}
