...
# uv upgrades the Python plugin, but not its separately cloned Node helper.
# Match that installed version, never upstream HEAD. The optional directory is
# for isolated tests; the default matches yt-dlp/config's server_home.
# A subshell keeps our cwd and cleanup traps away from sysup's inhibitor trap.
_sysup_bgutil() (
  emulate -L zsh
  trap - EXIT INT TERM
